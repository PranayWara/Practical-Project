...
@app.route('/')
def index():
    rarity = requests.get('http://service-2:5000/get/rarity').text
    gun = requests.get('http://service-3:5000/get/gun').text

    payload = {'rarity':rarity, 'gun':gun}
    price = requests.post('http://service-4:5000/post/winnings', json=payload).json()
